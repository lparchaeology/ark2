...
<xsl:stylesheet
    version="2.0"
    xmlns="http://www.concrete5.org/doctrine-xml/0.6"
...
>
    ...
    <xsl:template match="*">
      ...
      <xsl:element name="{$lowerCaseElementName}" namespace="http://www.concrete5.org/doctrine-xml/0.6">
          ...
          <xsl:for-each select="@*">
            ...
            <xsl:choose>
                ...
                <xsl:when test="$lowerCaseElementName = 'index'">
                    ...
                    <xsl:apply-templates select="node()[translate(name(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz') = 'spatial']" />
