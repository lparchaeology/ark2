...
/**
* subforms/sf_boolean_switch.php
*
...
function frmBooleanSwitch($field, $itemkey, $itemvalue=FALSE) {
    
    global $skin_path, $submis_serial, $form_method, $lang;
    
    $mk_false = getMarkup('cor_tbl_markup', $lang, 'false');
    $mk_true = getMarkup('cor_tbl_markup', $lang, 'true');
    
    // lets check we're doing it right
    if ($field['dataclass']!=='attribute'||!array_key_exists('attribute', $field)){
        echo "ADMIN ERROR: frmBooleanSwitch() The field {$field['field_id']} is not set correctly";
    }
    ...
    // set up the form
    $generic = "<form method=\"$form_method\" action=\"{$_SERVER['PHP_SELF']}\" class =\"bool\">\n";
    $generic .= "<input type=\"hidden\" name=\"submiss_serial\" value=\"{$_SESSION['submiss_serial']}\" />\n";
    $generic .= "<input type=\"hidden\" name=\"item_key\" value=\"$itemkey\" />\n";
    $generic .= "<input type=\"hidden\" name=\"$itemkey\" value=\"{$itemvalue}\" />\n";
    $generic .= "<input type=\"hidden\" name=\"itemval\" value=\"{$itemvalue}\" />\n";

    $change = "<input type=\"hidden\" name=\"update_db\" value=\"{$field['field_id']}\" />\n";
    $add = $change;
    $add .= "<input type=\"hidden\" name=\"{$field['classtype']}\" value=\"$attr_numeric\" />\n";
    $add .= "<input type=\"hidden\" name=\"{$field['classtype']}_qtype\" value=\"add\" />\n";
    
    $edit = $change;
    $edit .= "<input type=\"hidden\" name=\"{$field['classtype']}\" value=\"$attr_numeric\" />\n";
    $edit .= "<input type=\"hidden\" name=\"{$field['classtype']}_qtype\" value=\"edt\" />\n";
    $edit .= "<input type=\"hidden\" name=\"{$field['classtype']}_id\" value=\"{$current['frag_id']}\" />\n";
    
    $delete = "<input type=\"hidden\" name=\"update_db\" value=\"delfrag\" />\n";
    $delete .= "<input type=\"hidden\" name=\"dclass\" value=\"{$field['dataclass']}\" />\n";
    $delete .= "<input type=\"hidden\" name=\"delete_qtype\" value=\"del\" />\n";
    $delete .= "<input type=\"hidden\" name=\"frag_id\" value=\"{$current['frag_id']}\" />\n";
    ...
    if ($attr_bool === '1') {
        ...
        if ($set) {
            $true = $edit;
            $true .= "<input type=\"hidden\" name=\"{$field['classtype']}_bv\" value=\"1\" />\n";
        } else {
            $true = $add;
            $true .= "<input type=\"hidden\" name=\"{$field['classtype']}_bv\" value=\"1\" />\n";
        }
    }
    $true .= "<button class=\"$class\">";
    $true .= "<img src=\"$skin_path/images/onoff/chk_on.png\" alt=\"on\" />";
    $true .= "$mk_true</button>";
    $true .= "</button>";
    ...
    if ($attr_bool === '0') {
        ...
        if ($set) {
            $false = $edit;
            $false .= "<input type=\"hidden\" name=\"{$field['classtype']}_bv\" value=\"0\" />\n";
        } else {
            $false = $add;
            $false .= "<input type=\"hidden\" name=\"{$field['classtype']}_bv\" value=\"0\" />\n";
        }
    }
    $false .= "<button class=\"$class\">";
    $false .= "<img src=\"$skin_path/images/onoff/chk_off.png\" alt=\"off\" />";
    $false .= "$mk_false</button>";
    $false .= "</button>";
    
    $var = $generic.$true."</form>";
    $var .= $generic.$false."</form>";
...
switch ($sf_state) {
    ...
    case 's_max_edit':
        ...
        foreach ($fields as $key => $field) {
            ...
            $val = "<label>{$alias_classtype}</label>";
            $val .= frmBooleanSwitch($field, $sf_key, $sf_val);
            print($val);
            //close the form
            // close the list item
            print("</li>\n");
