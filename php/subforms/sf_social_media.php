...
/**
* subforms/sf_social_media.php
*
* subform for social media links
*
...
switch ($sf_state) {
	...
	case 's_max_view':
		...
		// put in the nav
		// print(sfNav($sf_title, $cur_col_id, $cur_sf_id, $$disp_cols));
		$var ='<label class="form_label"> Elsewhere on the Web</label>';
		if ($error) {
			feedBk('error');
		}
		if ($message) {
		...
		$var .= '<div class="sociallinks">
		<ul>';
		foreach ($fields as $field){
			
			$handle = getSingleText($sf_key, $sf_val, $field['classtype'], $lang);
			if ($field['classtype']=='twitter'){
				$var .= '
					<li>
					<a target="_blank" href="http://www.twitter.com/'.$handle.'">
					<span>Twitter</span>
					<img alt="Twitter" src="'.$skin_path.'/_images/common/icon-social-twitter.svg">
					</a>
					</li>';
			} else {
				//no other social media added yet
				/*
				 *  <li>
					 <a target="_blank" href="#">
					  <span>Facebook</span>
					  <img alt="Facebook" src="_images/common/icon-social-facebook.svg">
					 </a>
				   	</li>
					<li>
					 <a target="_blank" href="#">
					  <span>LinkedIn</span>
					  <img alt="LinkedIn" src="_images/common/icon-social-linkedin.svg">
					 </a>
					</li>
				 */
			}
