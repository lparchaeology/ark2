...
if ($update_db === $sf_conf['sf_html_id']) {
    include_once ('php/sgr_view/subforms/update_interp.php');
}
...
$mk_copyfromregister = getMarkup('cor_tbl_markup', $lang, 'copyfromregister');
$sf_title = getMarkup('cor_tbl_markup', $lang, $sf_conf['sf_title']);
...
switch ($sf_state) {
    ...
    case 'p_max_ent':
        ...
        if ($interp_texts) {
            ...
            foreach ($interp_texts as $interp) {
                ...
                if ($interp['id'] == $quickedit) {
                    ...
                    print("
                        <li class=\"row\">
                            ...
                            <span class=\"inp\"><button type=\"submit\">save</button></span>
                        </li>\n
                    ");
                    // end the list of fields
                    print("</ul>");
                ...
                // if not on quickedit print a view of the interp
                    ...
                    $qed = "<a class=\"btn\" href=\"{$_SERVER['PHP_SELF']}?$sf_key={$sf_val}&amp;quickedit={$interp['id']}\">";
                    $qed .= "$img</a>";
                    print("
                        <li class=\"row\">
                            <label class=\"form_label\">&nbsp;</label>
        ...
        if (!$quickedit) {
            ...
            // end the form
            print("</form>");
            if(array_key_exists('op_copy_button',$sf_conf)){
                            print("<button id=\"copyfromregister\">$mk_copyfromregister</button>");
                            $script = "<script> 
                                $(document).ready(function(){
                                    $('#copyfromregister').on('click', function(event){
                                        console.log($('#{$sf_conf['op_copy_button']}'));
                                        console.log($('#{$sf_conf['op_copy_button']}').find('.data').text());
                                        console.log($('#interp'));
                                        $('#{$fields[0]['classtype']}').val($('#{$sf_conf['op_copy_button']}').find('.data').text());
                                    });
                                });
                            </script>";
                            print $script;
