...
if (isset($sgr_view_left_panel) && is_array($sgr_view_left_panel)) {
    if (array_key_exists('subforms', $sgr_view_left_panel)) {
        $leftpanelmode = 'subforms';
    } elseif (array_key_exists('href', $sgr_view_left_panel[0])) {
...
switch($leftpanelmode) {
    case 'subforms' :
        ...
        $fake_cols[] = $sgr_view_left_panel;
        // This is always 0 in data entry as there is only a single col
        $cur_col_id = 0;
        // now loop over the sf's
        foreach ( $sgr_view_left_panel['subforms'] as $cur_sf_id => $sf_conf ) {
...
<?php 
    ...
    case 'err':
    default:
        echo "\$sgr_view_left_panel is not set correctly";
}
